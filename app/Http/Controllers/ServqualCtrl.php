...
class ServqualCtrl extends Controller
{
    ...
    public function nilai($id)
    {
        ...
        return view('servqual.nilai', compact('assign','nilai', 'criteria', 'nilaiDimensi', 'keterangan', 'dimensi'));
    }

    public function cetak_semua($id)
    {
        $company = Company::where('id', $id)->first();
        $assign = Assignment::where('company_id', $id)->with(['checker','company'])->first();
        $kuisioner = new Kuisioner;
        ...
        $criteria = Criteria::all();
        $dimensi = Dimension::with(['criteria'])->get();
        $patternFileName = $assign->company->name.'_'.$assign->checker->name.'.pdf';
    ...
    public function cetak_periode($id)
    {
        ...
        $criteria = Criteria::all();
        $dimensi = Dimension::with(['criteria'])->get();
        $patternFileName = $assign->company->name.'_'.$assign->bulan.'_'.$assign->tahun.'_('.$assign->checker->name.')_.pdf';
